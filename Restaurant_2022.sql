...
--Some restaurants have many items so their nutritional value's averages are more well-rounded, while some restaurants only had a few items so their averages are very skewed, either really high or low.
--Some item's nutritional values are accurate while there are some that are not. Another analysis may be needed after reconfirming the nutritional facts for each item from multiple sources.
--Categorization for several items are misleading and/or doesn't make the most logical sense. Item names have actual food items labelled correctly, but some are toppings/ingredients added on top of food, but have overwhelmingly high
--calories count, etc. 